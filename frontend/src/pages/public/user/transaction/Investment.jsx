...
const InvestmentTransaction = () => {
    ...
    const fetchTransactions = useCallback(async() => {
        try {
            const response = await fetch (`${import.meta.env.VITE_APP_API_URL}fetch/investment_transaction.php`, {
                method: 'GET',
                credentials: 'include',
                headers: {
    ...
    useEffect(() => {
        document.title = `Investment History - ${import.meta.env.VITE_APP_NAME}`;
        fetchTransactions();
    }, [fetchTransactions])
                ...
                <div className={styles['transaction_main_container']}>
                    {filteredData && filteredData.length > 0 ? (
                        filteredData.map((item) => {
                            ...
                            if (titleLower.includes('deposit') || titleLower.includes('profit')) {
                                sign = '+';
                                ...
                                <div key={item.id} className={`${styles['transaction_history_container']}`}>
                                    <div className="d-flex">
                                        <p className={sign === '+' ? `${styles['transaction_icon']} mx-2`: `${styles['transaction_icon_red']} mx-2`}>
                                            <span className="bi-arrow-down-left"></span>
                                        </p>
