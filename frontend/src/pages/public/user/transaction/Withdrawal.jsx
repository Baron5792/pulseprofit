...
const WithdrawalHistory = () => {
    ...
    const fetchTransactions = useCallback(async() => {
        try {
            const response = await fetch (`${import.meta.env.VITE_APP_API_URL}fetch/fetch_withdrawal.php`, {
                method: 'GET',
                credentials: 'include',
                headers: {
    ...
    useEffect(() => {
        document.title = `Withdrawal History - ${import.meta.env.VITE_APP_NAME}`;
        fetchTransactions();
    }, [fetchTransactions])
                        ...
                        filteredData.map((item) => (
                            <div key={item.id} className={`${styles['transaction_history_container']}`}>
                                <div className="d-flex">
                                    <p className={`${styles['transaction_icon_red']} mx-2`}>
                                ...
                                <div className={`${styles['transaction_amount']} d-flex`}>
                                    <p className="text-danger">+ {formatCurrency(item.amount)} <span className="text-secondary">USD</span></p>
