...
export const TransactionDeposit = () => {
    ...
    const fetchTransactions = useCallback(async() => {
        try {
            const response = await fetch (`${import.meta.env.VITE_APP_API_URL}fetch/deposit_transactions.php`, {
                method: 'GET',
                credentials: 'include',
                headers: {
    ...
    useEffect(() => {
        document.title = `Deposit History - ${import.meta.env.VITE_APP_NAME}`;
        fetchTransactions();
    }, [fetchTransactions])
                        ...
                        filteredData.map((item) => (
                            <div key={item.id} className={`${styles['transaction_history_container']}`}>
                                <div className="d-flex">
                                    <p className={`${styles['transaction_icon']} mx-2`}>
                                ...
                                <div className={`${styles['transaction_amount']} d-flex`}>
                                    <p className="text-success">+ {formatCurrency(item.amount)} <span className="text-secondary">USD</span></p>
                ...
                <Modal show={transactionModal}>
                    <div className={styles['modal_body']}>
                        ...
                        <div className={styles['modal_content']}>
                            ...
                            <div className={styles['others']}>
                                <p>Reference ID</p>
                                <p>{transaction && transaction.reference_id.length > 20 ? transaction.reference_id.slice(0, 20) + '...': transaction.reference_id } <button type="button" className='btn btn-normal bi bi-copy small p-0' onClick={() => copyReferenceId(transaction.reference_id)}></button></p>
                            </div>
